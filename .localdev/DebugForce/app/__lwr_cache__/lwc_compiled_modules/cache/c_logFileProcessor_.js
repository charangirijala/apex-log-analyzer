...
import _tmpl from "./logFileProcessor.html";
 ...
 */
class LogFileProcessor extends LightningElement {
  constructor(...args) {
    super(...args);
    this.STD_EXP_MATCHER = new RegExp("^[0-9:.]*\\s\\([0-9]*\\)(|)[A-Z_]*.*");
    this.EXE_ANONYMOUS_MATCHER = new RegExp("^(Execute\\sAnonymous:\\s).*");
    this.result = [];
  ...
  createMethodUnit(line, type, index) {
    ...
    if (type === "Method-Generic") {
      ...
      let s2 = "(" + methodUnit.methodTitle.substring(methodUnit.methodTitle.lastIndexOf("(") + 1);
      methodUnit.methodName = methodUnit.methodTitle.replace(s2, "").substring(methodUnit.methodTitle.lastIndexOf(".") + 1) + s2;
  ...
  addCUtoResult(codeUnit) {
    if (this.codeUnitsStack.length !== 0) {
      ...
      if (CUTop.childUnitsandLines === null || CUTop.childUnitsandLines === undefined) {
        CUTop.childUnitsandLines = [];
...
_registerDecorators(LogFileProcessor, {
  wire: {
    messageContext: {
  ...
  fields: ["STD_EXP_MATCHER", "EXE_ANONYMOUS_MATCHER", "result", "isCurUnitCU", "codeUnitsStack", "methodUnitsStack", "stdExpCount", "codeUnitsCount", "methodUnitsCount", "eventsPicklistValues", "execAnonyCount", "fileData", "fileMetadata", "fileDataPartial", "treeNodes", "level", "posinset", "maxsize"]
});
const __lwc_component_class_internal = _registerComponent(LogFileProcessor, {
  tmpl: _tmpl,
  sel: "c-log-file-processor",
  apiVersion: 62
});
